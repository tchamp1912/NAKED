...
void explore_status_callback(const actionlib_msgs::GoalStatusArrayConstPtr &status)
{
   ...
   if (size > 0)
   {
      ...
      else if (status_value == 4) // ABORTED
      {
         ...
         ROS_ERROR("Exploration failed, will not search for qr_code.");
      }
   }
}
...
void qr_code_found_callback(const std_msgs::String &msg)
{
   if (msg.data != "None")
   {
...
void set_new_goal()
{
   if (qr_code_search_in_progress)
   {
      ROS_INFO("Set new point for qr_code recognition.");
      ...
      else
      {
         ...
         {
            ROS_INFO("qr_code search finished, qr_code not found");
...
int main(int argc, char **argv)
{
   ...
   ros::Subscriber qr_code_found_sub = node.subscribe("/qr_code", 5, qr_code_found_callback);
   ...
   ROS_INFO("Begin searching for qr_code");
   qr_code_search_in_progress = true;
   set_new_goal();

   while (node.ok() && qr_code_search_in_progress && !qr_code_found)
   {
      update_robot_pos();
   ...
   if (node.ok())
   {
      ...
      {
         ROS_INFO("qr_code search stopped, all area checked, nothing found");
      }
      cancel_move_base_action();
      node.shutdown();
   }
